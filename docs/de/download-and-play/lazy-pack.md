...
1. **Vollständiges Lazy-Pack für Spieler ohne Kompilierung (`GTE-LazyPack-*.zip`)** : Enthält alle vorkompilierten Mods, Konfigurationen, Modifikationsskripte und die vollständige `.minecraft`-Verzeichnisstruktur. **Doppelklick oder Ziehen in den Launcher genügt zum Spielen.**
...
- **Vollständige Paketierung**: `gtecore`, `gtm-reborn`, `gt--` neueste Release-Jars und erforderliche Erweiterungsmods sind bereits im `mods/`-Verzeichnis enthalten.
...
=== "Methode 1: Launcher-Drag-and-Drop (empfohlen)"
    ...
    5. Weisen Sie **8GB ~ 12GB** Arbeitsspeicher zu und starten Sie das Spiel!

=== "Methode 2: Manueller Entpackmodus"
    ...
    4. Stellen Sie sicher, dass Sie **Java 21** als Kern auswählen und starten Sie.

---

## ⚠️ Java 21 Laufzeitumgebungsanforderung (sehr wichtig)

> [!CAUTION]
> **Dieses Integrationspaket erfordert zwingend Java 21 (JDK 21) als Laufzeitumgebung!**
> Verwenden Sie keinesfalls **Java 17** oder **Java 8**, da das Spiel sonst sofort abstürzt oder sich weigert zu starten!

### Warum ist Java 21 erforderlich?
- Die Kernmods von GTE (`gtecore`, `gtm-reborn`, `gt--`) nutzen umfassend **moderne Java-21-Sprachfunktionen** (wie Record Patterns, Virtual Threads, verbessertes Switch-Matching).
- Die Gradle-Build-Skripte konfigurieren global `JavaLanguageVersion.of(21)` zur erzwungenen Toolchain-Prüfung.

### Empfohlene JDK-21-Downloadadressen

| Distribution | Download-Link | Empfehlungsgrund |
| :--- | :--- | :--- |
| **Azul Zulu 21 (LTS)** | [Klicken Sie hier für die Azul-Website](https://www.azul.com/downloads/?version=java-21-lts) | Hervorragende Leistung, optimiert für groß angelegte Multithreading in Minecraft |
| **Eclipse Temurin 21 (LTS)** | [Klicken Sie hier für die Adoptium-Website](https://adoptium.net/temurin/releases/?version=21) | Offiziell empfohlen, hohe Kompatibilität und Stabilität |
| **Microsoft OpenJDK 21** | [Klicken Sie hier für die Microsoft-Website](https://learn.microsoft.com/zh-cn/java/openjdk/download) | Gute native Anpassung für Windows-Plattformen |
...
graph LR
    ...
    C --> D[Installierte JDK-21-javaw.exe auswählen]
    D --> E[8192MB ~ 12288MB Arbeitsspeicher zuweisen]
    E --> F[Speichern und Spiel starten]
```

---

## 🎮 In-Game-Tastenkürzel und häufig verwendete Befehle

| Befehl / Tastenkürzel | Funktionsbeschreibung | Berechtigungsanforderung |
| :--- | :--- | :--- |
| `/ftbquests editing_mode true` | Aktiviert den visuellen Bearbeitungsmodus für Questbücher (Autorenmodus) | OP-Berechtigung |
...
| `/dumpmultiblock` | Exportiert nach Auswahl mit der Holzaxt den Multiblock-Strukturcode mit einem Klick | OP-Berechtigung |
| <kbd>U</kbd> / <kbd>R</kbd> | Zeigt Verwendung (Usage) / Rezept (Recipe) des Elements unter dem Cursor an | EMI / JEI-Tastenkürzel |
| <kbd>F7</kbd> | Zeigt die Lichtstärke der Umgebung an (rotes Kreuz markiert Spawnbereiche) | Client-Tastenkürzel |