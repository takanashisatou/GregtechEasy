...
- **0 зависимостей от компиляции**: не требуется установка JDK, IntelliJ IDEA или Git.
...
- **Играй сразу после перетаскивания**: поддерживается импорт одним перетаскиванием в окно PCL2 / HMCL.

### Шаги по импорту и запуску

=== "Способ 1: Однокликовое перетаскивание в лаунчер (рекомендуется)"
...
- В скриптах сборки Gradle глобально задан `JavaLanguageVersion.of(21)` для принудительной проверки инструментария.

### Рекомендуемые ссылки для скачивания JDK 21

| Дистрибутив | Ссылка для скачивания | Причина рекомендации |
| :--- | :--- | :--- |
...
graph LR
    ...
    B --> C[Путь к Java / среда выполнения]
    C --> D[Выбрать установленный javaw.exe из JDK 21]
    D --> E[Выделить 8192–12288 МБ памяти]
...
| <kbd>F7</kbd> | Просмотр уровня освещённости вокруг (красный крест означает зону спавна мобов) | Клиентская горячая клавиша |