...
Помимо готового пакета без компиляции, GTE предоставляет стандартные пакеты CurseForge и серверные пакеты, автоматически собираемые на основе **Packwiz**.
...
### 3. Настройка скриптов запуска (`run_server.sh` / `run_server.bat`)
...
> **Причина**: версия Java во время выполнения сервера ниже Java 21 (версия 65.0 означает JDK 21).  
> **Решение**: на Linux переключитесь на OpenJDK 21 с помощью `sudo update-alternatives --config java`.

### Q2: При входе на сервер игроки получают сообщение о несоответствии списка модов
> **Решение**: убедитесь, что версия клиента и сервера полностью совпадает. Каждая сборка CI основного проекта синхронно генерирует соответствующие артефакты Client и Server.