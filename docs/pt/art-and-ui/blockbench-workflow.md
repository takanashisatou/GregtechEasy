# Interface, Texturas e Fluxo de Trabalho Artístico no Blockbench

O projeto GTE estabeleceu um pipeline automatizado e sem perdas para o processamento de ativos artísticos. Os designers de modelos precisam apenas usar o **Blockbench** para criar modelos e salvá-los no diretório de origem; as tarefas do Gradle cuidam automaticamente da classificação de ativos, validação de formato e sincronização incremental.

---

## 🎨 Diretório de Arquivos de Arte (`art_assets/`)

O diretório `art_assets/` na raiz do projeto é o **único diretório de trabalho** para designers de arte, rigorosamente versionado pelo Git:

```
art_assets/
├── *.bbmodel                           # Arquivos de projeto do Blockbench (preservando camadas e ossos)
├── *.json                              # Modelos geométricos do Minecraft exportados pelo Blockbench
...
## 🏷️ Convenções de Nomenclatura e Regras de Roteamento Automático

A tarefa do Gradle `syncBlockbenchAssets` distribui automaticamente os arquivos para os caminhos de recursos correspondentes em `modules/gtecore`, com base em palavras-chave no nome do arquivo:
...
1. **Disparo Automático**: Esta tarefa está vinculada aos nós anteriores de `buildAll`, `copyOutputJars` e do pipeline de CI, sendo executada automaticamente durante compilação local ou inicialização do jogo, sem necessidade de cópia manual repetida.
...
3. **Mantém o Git Limpo**: Os arquivos `.bbmodel` permanecem apenas em `art_assets/` como projeto de origem; o jar compilado não inclui metadados redundantes do projeto Blockbench.