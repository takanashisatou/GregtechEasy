...
1. **Pacote completo para jogadores sem compilação (`GTE-LazyPack-*.zip`)**: Inclui todos os mods pré-compilados, configurações, scripts de modificação e a estrutura completa do diretório `.minecraft`. **Basta clicar duas vezes ou arrastar para o launcher para jogar**.
2. **Pacote padrão CurseForge (`GTE-CurseForge-*.zip`)**: Formato padrão CurseForge, pode ser importado diretamente no PCL2 / HMCL / CurseForge App / Prism Launcher com um clique.
3. **Pacote de servidor (`GTE-Server-*.zip`)**: Inclui configurações de servidor limpas, mods e scripts de inicialização, para abrir servidores e jogar em rede.
...
- **Empacotamento completo**: Os JARs mais recentes de `gtecore`, `gtm-reborn`, `gt--` e os mods de extensão pré-requisitos já estão incluídos no diretório `mods/`.
...
=== "Método 1: Arrastar e soltar no launcher (Recomendado)"
    ...
    5. Aloque **8GB ~ 12GB** de memória e clique para iniciar o jogo!

=== "Método 2: Modo de extração manual"

    1. Extraia o arquivo compactado para qualquer caminho sem caracteres chineses e sem espaços (exemplo: `D:\Games\GTE\`).
    2. Após a extração, você obterá um diretório `.minecraft` contendo `mods/`, `config/`, `kubejs/`.
    3. No launcher, adicione uma versão do jogo e selecione o diretório `.minecraft` extraído como raiz do jogo.
...
### Por que é obrigatório usar Java 21?
- Os mods principais do GTE (`gtecore`, `gtm-reborn`, `gt--`) utilizam amplamente **recursos modernos da linguagem Java 21** (como Record Patterns, Virtual Threads, aprimoramento do Switch).
- O script de build do Gradle configura globalmente `JavaLanguageVersion.of(21)` para forçar a verificação da toolchain.

### Endereços de Download Recomendados para JDK 21
...
| `/kubejs reload server_scripts` | Recarrega scripts de modificação do servidor e receitas | Permissão de OP |
| `/kubejs reload client_scripts` | Recarrega scripts de modificação do cliente e lógica de exibição | Sem permissão necessária |
| `/dumpmultiblock` | Após selecionar uma área com o machado de madeira, exporta o código da estrutura multibloco com um clique | Permissão de OP |
