...
Esta guía está dirigida a programadores de Java/Kotlin y autores de modpacks que participan en el desarrollo del proyecto de ingeniería entre módulos GTE-Multi.
...
- **Minecraft Development**: proporciona sugerencias de código Mixin, reconocimiento de transformadores de acceso (AT) y resaltado de eventos.
...
1. En IDEA, haga clic en **Archivo ➜ Abrir** y seleccione el `build.gradle` en el directorio raíz para abrirlo como proyecto.
2. Vaya a la configuración: `Configuración` ➜ `Compilación, ejecución, implementación` ➜ `Herramientas de compilación` ➜ `Gradle`.
3. Especifique **Gradle JVM** como **JDK 21**.
...
# 4. Compilar todos los módulos y empaquetar los Jars de una sola vez
...
# 7. Publicar todos los módulos como artefactos estáticos en build/maven (para Maven de GitHub Pages)
