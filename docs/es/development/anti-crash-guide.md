...
### Regla de oro 2: Prohibido colocar mods de optimización/shaders de entorno de producción en el entorno de desarrollo

- **Causa raíz del fallo**: Mods de optimización de entorno de producción como `Oculus`, `Embeddium`, `ModernFix`, `ModernUI` incluyen mapeos Mixin SRG codificados (como `f_117950_`, `m_91302_`). Sin embargo, el entorno de desarrollo Gradle `runClient` se ejecuta con mapeos Mojang desofuscados, lo que provoca directamente un fallo `InvalidMixinException`.
- **Principio de gestión**: Colocar los mods de optimización en `gte/overrides/mods/` (para uso con lanzadores normales), y prohibir añadirlos como dependencias de construcción de `modules/gte-dev-runtime`.

---

### Regla de oro 3: Las dependencias del entorno de desarrollo deben usar uniformemente `modLocalRuntime`

- **Causa raíz del fallo**: Un `localRuntime` normal o `fileTree` no activa el remapeador de ofuscación de ModDevGradle, lo que provoca que no se encuentren símbolos o que los nombres ofuscados se rompan en tiempo de ejecución.
...
- **Síntoma**: Al ejecutar `compileJava` o `build`, se muestra `NoSuchFileException: ...\build\classes\java\main\...` o `Unable to delete directory 'build'`.
...
Cuando se modifican las máquinas base, el sistema de materiales, los RecipeType, las condiciones de recetas o las Capabilities de `gtm-reborn`, se deben ejecutar secuencialmente los siguientes tres pasos de verificación:
...
3. **Verificar las referencias de objetos de FTB Quests**: Revisar si el libro de misiones referencia IDs de objetos que fueron renombrados o eliminados.

---

## 📚 Base de análisis post-mortem y recetas de reparación de fallos reales (Post-Mortems)
...
- **Solución**: Reemplazar por operación de flujo nativa:
  ```java
...
- **Solución**: El Map estático de `RegistrateDataProvider` solo se inicializa con el parámetro `--datagen`. Envolver la llamada en `try { ... } catch (Throwable ignored) { }` para evitar errores en el arranque normal.

### Caso 4: Falta `PonderPlugin` y se produce `NoClassDefFoundError`
- **Pila de errores**: `GTMachines.<clinit>` lanza `NoClassDefFoundError: PonderPlugin`, luego Ponder falla indicando `requires flywheel`
- **Solución**: En `modules/gte-dev-runtime/build.gradle`, añadir tanto `modLocalRuntime(forge.ponder)` como `modLocalRuntime(forge.flywheel.forge)`.