...
GTE delega la mayor parte del registro de materiales, ajustes de recetas y la lógica de integración entre múltiples mods a **KubeJS** (directorio ubicado en `gte/overrides/kubejs/`).
...
├── startup_scripts/     # 【Scripts de inicio】Se ejecutan en la fase más temprana del juego, utilizados para registrar materiales, fluidos, bloques y objetos
├── server_scripts/      # 【Scripts del servidor】Se ejecutan al entrar en una partida/conectarse al servidor, utilizados para registrar/modificar recetas y etiquetas
├── client_scripts/      # 【Scripts del cliente】Se ejecutan en el cliente, utilizados para modificar Tooltips, la visualización de la interfaz JEI/EMI
...
GTCEuStartupEvents.registry('gtceu:material', event => {
    // 1. Registrar el metal infinito (Infinite)
    ...
    // 2. Registrar el metal de fluido oscuro (Dark Fluid)
    ...
    // 3. Registrar la materia Meow Meow (Meow Meow Matter) y la antimateria (Antimatter)
...
ServerEvents.recipes(event => {
    ...
    // Eliminar recetas ineficientes originales
