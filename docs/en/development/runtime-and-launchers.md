# Local Hot Reload and Launcher-Free Quick Run

GTE has designed a seamless hot-reload system that is extremely friendly to pack planners, quest writers, and mod programmers.

---

## ⚡ 1. Launcher-Free Quick Start Script (`run_game.bat` / `run_game.sh`)

For quest writers (FTB Quests) and KubeJS recipe planners, **no need to open IntelliJ IDEA or install any third-party launcher**; simply double-click **`run_game.bat`** in the project root to enter the game instantly!
...
### Core Features
1. **Fully automatic JDK 21 detection**: Automatically scans for Java 21 installed under `.jdks`, `Adoptium`, `Zulu`, and `Program Files`, and remembers it in `.jdk_path`.
...
3. **Zero-move workflow**: Modify quests in-game (`/ftbquests editing_mode true`) and save; changes are directly saved in real-time to the corresponding `config/ftbquests/` in the Git repository. Open GitHub Desktop and commit with one click!

---

## 🔗 2. External Launcher Zero-Copy Mapping Tool (`link_to_launcher.bat`)

If you prefer using a launcher with your own configured skins and keybindings (such as PCL2 / HMCL / Prism Launcher):
...
- **Purpose**: A purely local hot-compile debugging sandbox; **prohibited from packaging and release, and will not appear in any player builds**.
