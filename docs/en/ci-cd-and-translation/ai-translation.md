...
The GTE project implements an industrial-grade multilingual internationalization translation system driven by a unified script, covering three major areas: Mod assets, FTB quests, and Markdown documentation.

---

## 🔒 Five Iron Rules for Translation

This project's translation work adheres to the following **5 inviolable iron rules**:

1. **Single Script**: All translations are driven solely by `scripts/opencode_translate.py`, which integrates with OpenCode Zen's `deepseek-v4-flash` model. Introducing a second translation script or manually assembling API calls is prohibited.
...
3. **Single Deployment**: The entire site is uniformly deployed to `https://takanashisatou.github.io/GregtechEasy/` (`gh-pages` branch). No second documentation site or duplicate deployment is allowed.
4. **English Rules**:
...
Traditional community localization relies on manual maintenance of complex JSON and SNBT texts, leading to delayed updates and frequent errors.

GTE's AI translation engine, through a standardized OpenAI-compatible API, achieves **automated incremental extraction, terminology alignment, and concurrent translation** for FTB Quests and core Mod language files:
...
The script automatically selects the first available API Key according to the following priority, without requiring manual provider specification:
...
| 6 | Zhipu Qingyan (Zhipu GLM) | `ZHIPU_API_KEY` | `ZHIPU_BASE_URL` | `glm-4-flash` |
| 7 | OpenAI | `OPENAI_API_KEY` | `OPENAI_BASE_URL` | `gpt-4o-mini` |
| 8 | Generic Aggregation Proxy | `LLM_API_KEY` | `LLM_BASE_URL` | `LLM_MODEL` (custom) |

> **Note**: Only `OPENCODE_API_KEY` needs to be configured in GitHub Secrets for the CI to run completely. The rest are backup Failover.
...
4. **Mermaid Diagram Text Localization**: Flowchart node labels (e.g., `A[Label]`) are translated to the target language, while syntax keywords such as `graph TD`, `-->`, `subgraph` remain unchanged.
...
| :--- | :--- | :--- |
| Automatic full build and translation after code push | `sync-build.yml` | Push to `main`/`master` triggers automatically |
| Automatic translation and deployment on documentation changes | `docs-deploy.yml` | Triggered when `docs/` or `mkdocs.yml` changes |
| Manual full-scale mod asset translation | `translate.yml` | Manually triggered on Actions page, optional Provider and language |
| Manual full-scale documentation translation | `translate.yml` | Check the `translate_docs` input item |

> [!CAUTION]
> It is prohibited to manually run `python scripts/opencode_translate.py` locally for large-scale full translation. Local execution is only for debugging a single file or verifying API Key connectivity.