# KubeJS 도구 세트 및 멀티블록 내보내기 (`/dumpmultiblock`)
...
사용자 정의 멀티블록(Java 코드든 KubeJS 스크립트든)을 개발할 때 수십 개의 문자 레이어로 구성된 `FactoryBlockPattern.aisle(...)`을 수동으로 작성하는 것은 매우 시간이 많이 걸리고 오류가 발생하기 쉽습니다.

GTE에는 **`/dumpmultiblock` 목재 도끼 선택 영역 내보내기** (`server_scripts/easymultiblock.js`)가 내장되어 있습니다:
...
GTE는 KubeJS를 통해 전 차원 유체 및 가스 수집을 확장했습니다:
...
- **네더 지옥 기체**: `circuit(5)` ➜ 출력 `gtceu:nether_air 10000`
- **엔드 보이드 기체**: `circuit(6)` ➜ 출력 `gtceu:ender_air 10000`

### 2. 만능 회로 변환기 (`universal_circuit.js`)
크로스 모드 및 다양한 등급의 회로 기판에 대한 복잡한 레시피 중첩 문제를 해결하기 위해 GTE는 **범용 회로 (`universal_circuit`)** 시스템을 도입했습니다:
