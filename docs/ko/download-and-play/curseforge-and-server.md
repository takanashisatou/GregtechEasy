...
=== "PCL2 / HMCL 가져오기"
    ...
    3. 런처가 자동으로 `manifest.json`을 분석하고 고속 동시 다운로드로 의존 모드를 내려받습니다.
...
> **해결**: Linux에서 `sudo update-alternatives --config java`를 실행하여 OpenJDK 21로 전환합니다.

### Q2: 플레이어가 서버에 접속할 때 모드 목록 불일치 메시지가 표시됨
> **해결**: 클라이언트 버전 번호와 서버 버전 번호가 완전히 일치하는지 확인하세요. 메인 프로젝트 CI 빌드마다 클라이언트 및 서버 구성 요소가 동시에 생성됩니다.