# CurseForge 가져오기 및 서버 배포 가이드

컴파일 없는 편의 팩 외에도 GTE는 **Packwiz** 기반으로 자동 구축된 CurseForge 규격 팩과 서버 팩을 제공합니다.

---

## 📦 CurseForge 규격 팩 가져오기

CurseForge 형식 통합 팩 파일 이름은 `GTE-CurseForge-<버전>.zip`입니다.

### 클라이언트 가져오기 방법

=== "PCL2 / HMCL 가져오기"

    1. 런처를 열고 **새 게임 버전 설치 / 통합 팩 가져오기**를 선택합니다.
    2. 다운로드한 `GTE-CurseForge-<버전>.zip` 파일을 선택합니다.
    3. 런처가 `manifest.json`을 자동으로 분석하고 고속 동시 다운로드로 의존 모드를 다운로드합니다.
    4. 가져오기가 완료되면 버전 설정에서 Java 런타임을 **Java 21**로 지정합니다.
    5. 실행 메모리를 설정하고(권장 8GB ~ 12GB) 게임을 시작합니다.

=== "CurseForge App 가져오기"

    1. CurseForge App 클라이언트를 엽니다.
    2. 왼쪽의 **Minecraft** 아이콘을 클릭하고 **My Modpacks**로 이동합니다.
    3. 오른쪽 상단 설정 메뉴에서 **Create Custom Profile** ➜ **Import**를 클릭합니다.
    4. `GTE-CurseForge-<버전>.zip`을 선택하고 자동 다운로드 및 설치가 완료될 때까지 기다립니다.

=== "Prism Launcher 가져오기"

    1. **Add Instance (인스턴스 추가)** ➜ **Import (가져오기)**를 클릭합니다.
    2. `GTE-CurseForge-<버전>.zip`을 찾아 선택합니다.
    3. 인스턴스 생성 후 인스턴스 속성에서 Java를 **JDK 21** 경로로 설정합니다.

---

## 🖥️ 서버 배포 가이드

서버 파일 팩 이름은 `GTE-Server-<버전>.zip`입니다.

### 1. 환경 준비
- 운영 체제: Linux (Ubuntu 22.04+ / Debian 12+) 또는 Windows Server 2022+
- **JDK 21이 준비되어 있어야 합니다**: 터미널에서 `java -version`을 실행하여 출력이 `openjdk version "21..."`인지 확인합니다.
- 권장 사양: 4코어 CPU 이상, 16GB 물리 메모리 (Minecraft 서버에 10G ~ 14G 할당).

### 2. 배포 단계

```bash
# 1. 서버 작업 디렉터리 생성
mkdir -p /opt/gte-server && cd /opt/gte-server

# 2. 서버 팩 압축 해제
unzip GTE-Server-*.zip -d .

# 3. Forge 1.20.1-47.3.0 / 47.4.4 서버 코어 설치 (사전 설치되지 않은 경우)
#    설치 스크립트를 실행하여 minecraft_server 및 forge 라이브러리 다운로드
java -jar forge-1.20.1-*-installer.jar --installServer

# 4. Minecraft EULA 동의
echo "eula=true" > eula.txt
```

### 3. 시작 스크립트 구성 (`run_server.sh` / `run_server.bat`)

Aikar 최적화 매개변수를 사용하여 서버를 시작하는 것을 권장합니다:

=== "리눅스 (`run_server.sh`)"

    ```bash
    #!/bin/bash
    JAVA_CMD="java"
    MEMORY="12G"

    FLAGS="-Xms${MEMORY} -Xmx${MEMORY} \
      -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 \
      -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch \
      -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1ReservePercent=20 \
      -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 \
      -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 \
      -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1"

    $JAVA_CMD $FLAGS @libraries/net/minecraftforge/forge/1.20.1-47.3.0/unix_args.txt nogui
    ```

=== "윈도우 (`run_server.bat`)"

    ```bat
    @echo off
    set JAVA_CMD=java
    set MEMORY=12G

    set FLAGS=-Xms%MEMORY% -Xmx%MEMORY% -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch

    %JAVA_CMD% %FLAGS% @libraries/net/minecraftforge/forge/1.20.1-47.3.0/win_args.txt nogui
    pause
    ```

---

## ⚙️ 자주 묻는 질문 (FAQ)

### Q1: 서버 시작 시 `UnsupportedClassVersionError: ... class file version 65.0` 오류가 발생합니다
> **원인**: 서버 실행 시 Java 버전이 Java 21보다 낮습니다 (버전 65.0은 JDK 21을 의미).  
> **해결**: Linux에서 `sudo update-alternatives --config java`를 사용하여 OpenJDK 21로 전환합니다.

### Q2: 플레이어가 서버에 접속할 때 모드 목록 불일치 메시지가 표시됩니다
> **해결**: 클라이언트 버전과 서버 버전이 완전히 일치하는지 확인하세요. 메인 프로젝트 CI 빌드마다 클라이언트와 서버 구성 요소가 동시에 생성됩니다.