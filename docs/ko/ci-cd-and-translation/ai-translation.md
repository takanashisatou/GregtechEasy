...
이 프로젝트의 번역 작업은 다음 **5가지 불가침 철칙**을 따릅니다:
...
3. **유일한 배포 위치**: 전체 사이트는 `https://takanashisatou.github.io/GregtechEasy/` (`gh-pages` 브랜치)에 통합 배포되며, 두 번째 문서 사이트를 만들거나 중복 배포하지 않습니다.
4. **영어 규칙**:
   - 문서 시스템 (`docs/en/`): 영어는 AI가 `docs/zh/`에서 전체 번역해야 하며, 수동 덮어쓰기를 금지합니다;
   - 모드 프로젝트: `gtecore`의 `en_us.json`만 수동 유지되며, 스크립트에 보호 로직이 내장되어 기계 번역으로 덮어쓰지 않습니다.
5. **심층 현지화**: 내비게이션 메뉴 (`nav_translations`), Mermaid 흐름도 텍스트, 코드 주석, 표 레이블은 100% 해당 언어로 현지화되어야 합니다.

---

## 🤖 번역 엔진 아키텍처

전통적인 커뮤니티 현지화는 복잡한 JSON 및 SNBT 텍스트를 수동으로 유지하는 데 의존하여 업데이트가 지연되고 오류가 발생하기 쉽습니다.
...
스크립트는 다음 우선순위에 따라 사용 가능한 첫 번째 API Key를 자동으로 선택하며, 공급업체를 수동으로 지정할 필요가 없습니다:

| 우선순위 | 공급업체 이름 | API Key 환경 변수 | Base URL 환경 변수 | 기본 모델 |
| :---: | :--- | :--- | :--- | :--- |
| **1 (최우선)** | **OpenCode Zen** | `OPENCODE_API_KEY` | `OPENCODE_BASE_URL` | **`deepseek-v4-flash`** |
...
1. **형식 코드 절대 보존**: Minecraft 기본 색상 형식 코드 (예: `§a`, `§c`, `§6`) 및 자리 표시자 (`%s`, `%d`, `{0}`)를 완전히 보존합니다.
...
4. **Mermaid 다이어그램 텍스트 현지화**: 흐름도 노드 레이블 (예: `A[레이블]`)을 대상 언어로 번역하며, `graph TD`, `-->`, `subgraph` 등의 구문 키워드는 변경하지 않습니다.
5. **코드 주석 및 표 레이블**: 코드 블록 내 주석 (`//` / `#`) 및 표 열 제목을 전체 현지화합니다.
...
| 경로 | 보호 이유 | 보호 메커니즘 |
| :--- | :--- | :--- |
| `modules/gtecore/src/main/resources/assets/gtecore/lang/en_us.json` | gtecore 영어 번역은 작성자가 수동으로 유지 관리합니다. | 스크립트가 `is_gtecore` 플래그를 감지하여 `en_us` 언어는 덮어쓰기를 건너뜁니다. |
